...
#
# Extends the Phase 1 pinecone_service.py — reuses the same
# Pinecone connection and embedding model.
#
# Workflow for candidate embedding:
#   1. Load ResumeData for the candidate
#   2. Build combined embedding text from all resume fields
#   3. Generate 384-dim embedding using sentence-transformers
#   4. Upsert vector into Pinecone with metadata
#   5. Save EmbeddingMetadata record in SQLite
# ============================================================
...
def generate_candidate_embedding(
    ...
    if existing and existing.is_synced and not force and not resume_data.embedding_needs_update:
        logger.info(f"Candidate {candidate_profile_id} embedding already in sync, skipping")
        return {
    ...
    db.commit()
    logger.info(f"Candidate {candidate_profile_id} embedding synced: {vector_id}")
...
# ============================================================
def generate_job_embedding(db: Session, job_id: int, force: bool = False) -> dict:
    ...
    if existing and existing.is_synced and not force and not job.embedding_needs_update:
        return {
            "entity_type": "JOB",
            "entity_id":   job_id,
            "vector_id":   vector_id,
...
def sync_all_pending_embeddings(db: Session) -> dict:
    ...
    for job in pending_jobs:
        try:
            ...
            failed.append({"type": "job", "id": job.id, "error": str(e)})
    ...
    for resume in pending_resumes:
        try:
            generate_candidate_embedding(db, resume.candidate_profile_id, force=True)
            synced_candidates.append(resume.candidate_profile_id)
        except Exception as e:
            failed.append({
                "type": "candidate",
    ...
    return {
        ...
        "failed":            failed,
        "message":           f"Synced {len(synced_jobs)} jobs, {len(synced_candidates)} candidates"
    }
...
def get_embedding_status(db: Session) -> dict:
    ...
    jobs_synced = db.query(Job).filter(Job.embedding_needs_update == False).count()  # noqa

    total_cands   = db.query(ResumeData).count()
    cands_synced  = db.query(ResumeData).filter(ResumeData.embedding_needs_update == False).count()  # noqa
